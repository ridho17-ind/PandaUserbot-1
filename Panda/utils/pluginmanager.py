...
from Panda import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        name = "Panda.plugins.{}".format(shortname)
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    pandaub.remove_event_handler(i)
    ...
    try:
        name = f"Panda.plugins.{shortname}"
        for i in reversed(range(len(pandaub._event_builders))):
            ev, cb = pandaub._event_builders[i]
            if cb.__module__ == name:
                del pandaub._event_builders[i]
    except BaseException:
        raise ValueError
