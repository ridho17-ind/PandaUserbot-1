...
async def iytdl_inline(event):
    ...
    if not input_url:
        return await edit_delete(
            event, "Berikan masukan atau balas URL youtube yang valid"
        )
    pandaevent = await edit_or_reply(event, f"🔎 Searching Youtube for: `'{input_url}'`")
...
async def ytdl_download_callback(c_q: CallbackQuery):  # sourcery no-metrics
    ...
    callback_continue = f"Downloading {media_type} Mohon Tunggu..."
