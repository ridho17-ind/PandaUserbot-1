...
if Config.ANTISPAMBOT_BAN:
    ...
    async def anti_spambot(event):  # sourcery no-metrics
        ...
        if is_gbanned(user.id):
            pandagban = get_gbanuser(user.id)
            if pandagban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{pandagban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not pandabanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await pandaevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await pandaevent.edit("`CAS check failed`")
        return
...
async def caschecker(event):
    ...
    try:
        ...
        await pandaevent.edit("`spamwatch check failed: Admin privileges are required`")
        return
    except BaseException:
        await pandaevent.edit("`spamwatch check failed`")
        return
    await pandaevent.edit(text)
