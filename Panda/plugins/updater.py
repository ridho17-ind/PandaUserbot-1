...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n`PANDAUSERBOT is`  **up-to-date**  `with`  "
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"do `{cmdhd}update deploy` to update the pandauserbot"
        )
...
async def variable(var):
    ...
    await edit_or_reply(var, f"`Tunggu sebentar 2-3 minutes.`")
    heroku_var["UPSTREAM_REPO"] = "panda"
