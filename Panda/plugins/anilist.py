...
async def get_manga(event):
    ...
    await edit_or_reply(event, "`Searching Anime..`")
    jikan = jikanpy.jikan.Jikan()
...
async def character(event):
    ...
    await edit_or_reply(event, "`Searching Character...`")
    try:
        search_result = jikan.search("character", search_query)
    except APIException:
        return await edit_delete(pandaevent, "`Character not found.`")
    first_mal_id = search_result["results"][0]["mal_id"]
...
async def whatanime(event):
    ...
    output = await _pandatools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
    ...
    panda = f"https://telegra.ph{response[0]}"
