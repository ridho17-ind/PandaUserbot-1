...
def verify_cond(pandaarray, text):
    return any(i in text for i in pandaarray)


def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"pandauserbot_{userid}_{pack}_anim"
    return f"pandauserbot_{userid}_{pack}"
...
async def kang(args):  # sourcery no-metrics
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            pandaevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        if (
            ...
            await pandaevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    if user.username:
        ...
        try:
            ...
            username = f"panda_{user.id}"
    photo = None
    userid = user.id
    is_anim = False
    emoji = None
    reply = await event.get_reply_message()
    panda = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(pandaevent, "`Unsupported File!`")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, pandapackname = await newpacksticker(
                        pandaevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("Stickers") as conv:
                    pack, pandapackname = await add_to_pack(
                        pandaevent,
                        conv,
                        event,
                        packname,
            ...
            if pandapackname not in blablapacks:
                blablapacks.append(pandapackname)
...
async def pic2packcmd(event):
    ...
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "PandaUserbot_" + "".join(
        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)
    )
    image = await _pandatools.media_to_pic(pandaevent, reply, noedits=True)
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    pandaevent = await event.edit("__Making the pack.__")
...
async def get_pack_info(event):
    ...
    if not event.is_reply:
        return await edit_delete(event, "`Saya cuma dapat yang ini hiks  ?!`", 5)
    rep_msg = await event.get_reply_message()
    if not rep_msg.document:
        return await edit_delete(
            event, "`Balas ke sticker capek kali aku ngasih tau kau`", 5
        )
    try:
        stickerset_attr = rep_msg.document.attributes[1]
        pandaevent = await edit_or_reply(event, "`sedang mencari, Mohon tunggu..`")
    except BaseException:
        return await edit_delete(event, "`🐼 Mohon balas ke sticker.🐼`", 5)
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await pandaevent.edit("`🐼 Mohon balas ke sticker Panda 🐼.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    "Membuat nama pack sticker :)."
    split = event.pattern_match.group(1)
    if not split:
        return await edit_delete(
            event, "`Berikan beberapa nama untuk mencari paket.`", 5
        )
    pandaevent = await edit_or_reply(event, "`🐼 Mencari  sticker packs Panda 🐼....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "Sticker-pack__Panda"})
    if not results:
        return await edit_delete(pandaevent, "`Tidak dapat ditemukan :(.`", 5)
    reply = f"**🐼 Sticker Pack Panda 🐼 {split} are :**"
    for pack in results:
        if pack.button:
