...
PP_ERROR = "`Maaf gagal memproses gambar hiks:)`"
NO_ADMIN = "`Saya Bukan admin jangan dipaksa`"
NO_PERM = "`Saya tidak memiliki izin yang memadai! Ini sangat sial. Alexa bermain Bucinan hiks :)`"
CHAT_PP_CHANGED = "`Chat Picture Changed`"
INVALID_MEDIA = "`Gagal terjadi kesalahan :)`"
...
async def promote(event):
    ...
    if not user:
        return
    pandaevent = await edit_or_reply(event, "`Promoting...`")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await pandaevent.edit(NO_PERM)
...
async def demote(event):
    ...
    if not user:
        return
    pandaevent = await edit_or_reply(event, "`Demoting...`")
    ...
    try:
        ...
        return await pandaevent.edit(NO_PERM)
    await pandaevent.edit("`Demoted Successfully! Betterluck next time`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#DEMOTE\
...
async def _ban_person(event):
    ...
    pandaevent = await edit_or_reply(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    pandaevent = await edit_or_reply(event, "`Unbanning...`")
    try:
        ...
        await pandaevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
...
async def endmute(event):
    ...
    if not user:
        return
    pandaevent = await edit_or_reply(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await pandaevent.edit(NO_PERM + f"\n{str(e)}")
    if reason:
        await pandaevent.edit(
...
async def pin(event):
    ...
    if not to_unpin and options != "all":
        return await edit_delete(
            event,
            "__Balas ke pesan yang ingin diunpin __`.unpin all`__ to unpin all__",
            5,
        )
    try:
        if to_unpin and not options:
            ...
            return await edit_delete(
                event, "`balas ke pesan atau media yang ingin diupin`", 5
            )
    except BadRequestError:
        return await edit_delete(event, NO_PERM, 5)
    except Exception as e:
...
async def _iundlt(event):  # sourcery no-metrics
    "Untuk memeriksa pesan yang dihapus baru-baru ini di grup"
    pandaevent = await edit_or_reply(event, "`Mencari tindakan terbaru .....`")
    ...
    deleted_msg = f"**Baru {lim} Menghapus pesan(s) dalam grup ini adalah :**"
    if not flag:
        for msg in adminlog:
            ruser = (
        ...
        await edit_or_reply(pandaevent, deleted_msg)
    else:
