...
@pandaub.ilhammansiz_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec panda stringsetup.py",
    },
)
async def _(event):
    ...
    pandaevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    pandauser = await event.client.get_me()
    curruser = pandauser.username or "pandauserbot"
    uid = os.geteuid()
    if uid == 0:
...
@pandaub.ilhammansiz_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('pandauserbot')",
    },
)
async def _(event):
    ...
    pandaevent = await edit_or_reply(event, "`Running ...`")
